		...
		*,
		...
		try:
			...
			if self.is_syntax_error(e):
				...
				self.db_type in ("mariadb", "mysql") and frappe.log_error("Query deadlocked", defer_insert=True)
				raise frappe.QueryDeadlockError(e) from e

			elif self.is_timedout(e):
				self.db_type in ("mariadb", "mysql") and frappe.log_error("Query timed out", defer_insert=True)
